// ------ Compliant (Seed 043)
// Context: Embedded Sensor Sampling — applying gain and offset calibration
// Reference: MISRA C++ 2008 Rule 2-13-4 — Literal suffixes shall be upper case.
// Fix: All literal suffixes changed to uppercase 'F', 'U', and 'L'.

#include <iostream>
#include <vector>
#include <numeric>
#include <iomanip>

struct SensorConfig {
    float gain;         
    long offset;        
    unsigned buffer;    
};

class Sensor {
public:
    explicit Sensor(const SensorConfig& cfg)
        : cfg_(cfg), data(cfg.buffer, 0.0F) {}

    void captureSamples() {
        for (unsigned i = 0U; i < cfg_.buffer; ++i) {
            data[i] = static_cast<float>(i % 100U) * cfg_.gain + static_cast<float>(cfg_.offset);
        }
    }

    float averageReading() const {
        float sum = std::accumulate(data.begin(), data.end(), 0.0F);
        return sum / static_cast<float>(cfg_.buffer);
    }

    void printSummary() const {
        std::cout << "Gain: " << cfg_.gain 
                  << " Offset: " << cfg_.offset 
                  << " Buffer: " << cfg_.buffer 
                  << " Avg: " << std::fixed << std::setprecision(2) << averageReading() << "\n";
    }

private:
    SensorConfig cfg_;
    std::vector<float> data;
};

int main() {
    SensorConfig cfg{2.5F, -15L, 512U};
    Sensor sensor(cfg);
    sensor.captureSamples();
    sensor.printSummary();
    return 0;
}