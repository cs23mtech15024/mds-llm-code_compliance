...
// Compliance: namespace-scoped identifier renamed to avoid leading underscore — complies with MISRA C++:2008 Rule 17-0-1.
...
namespace drdo {
    namespace crypto {
        // COMPLIANT: namespace-scoped identifier without leading underscore
        ...
        void configureSystem(int keyValue) {
            internalConfig = (keyValue * 3 + 7) % 100;
        }
    }
}

void run_crypto_operations_C() {
    std::ofstream log("crypto_c.log", std::ios::app);
    if (!log) {
    ...
    for (size_t i = 0; i < testMessages.size(); ++i) {
        ...
        log << "[C] msg=" << i << " config=" << drdo::crypto::internalConfig 
            << " orig=" << original << " enc=" << encrypted << " dec=" << decrypted << '\n';
        
        std::cout << "[C] Message " << i << ": " << original << " -> " << encrypted 
                  << " -> " << decrypted << " (config=" << drdo::crypto::internalConfig << ")\n";
...
int main() {
    std::cout << "[C] Crypto utilities initialized\n";
    run_crypto_operations_C();
    std::cout << "[C] Crypto operations complete\n";
    return 0;
}