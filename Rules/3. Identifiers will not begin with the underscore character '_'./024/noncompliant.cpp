...
// Violation: macro identifier begins with '_' — may be reserved (MISRA C++:2008 Rule 17-0-1).
...
// NON-COMPLIANT: macro with leading underscore (seed)
...
namespace drdo {
    ...
    inline void logMessage(LogLevel level, const std::string& message, std::ofstream& log) {
        if (static_cast<int>(level) <= _DEBUG_LEVEL) {
            ...
            std::string logEntry = "[NC] " + timestamp + " " + levelToString(level) + ": " + message;
            std::cout << logEntry << '\n';
            log << logEntry << '\n';
        }
    }
...
void run_system_diagnostics_NC() {
    std::ofstream log("diagnostics_nc.log", std::ios::app);
    if (!log) {
...
int main() {
    std::cout << "[NC] Debug level configured to: " << _DEBUG_LEVEL << '\n';
    run_system_diagnostics_NC();
    return 0;
}