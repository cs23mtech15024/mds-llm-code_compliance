...
class SensorLogger_C {
public:
    SensorLogger_C(const std::string &outFile = "sensor_c.log")
    ...
    void runOnce() {
        // Compliant timing constant (uppercase 'U')
        constexpr unsigned INTERVAL_MS = 10U; // C
        int reading = sampleSensor();
        if (logfile) {
            logfile << "[C] reading: " << reading << "\n";
        } else {
            std::cout << "[C] reading: " << reading << "\n";
        }
...
int main() {
    SensorLogger_C logger("sensor_c.log");
