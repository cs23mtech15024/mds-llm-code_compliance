...
TASK (Pair 6 — literal suffix uppercase):
...
- Both files must:
  ...
  - Be ~50 lines each (±10).
  - Demonstrate polynomial evaluation in a template-based approach where the seed literal is used.
...
VALIDATION HINTS:
1. NC must contain `2u`; C must contain `2U` and not `2u`.
...
OUTPUT:
Return the two code blocks (NC then C) and a 1–2 sentence explanation naming the literal(s) changed and the rule(s) are satisfied.
END.
"""