...
class PolynomialEvaluator_NC {
public:
    PolynomialEvaluator_NC(const std::array<T, N>& coeffs) : coefficients(coeffs) {}
    ...
    void demo() const {
        // Seed usage: lowercase 'u'
        unsigned step = 2u; // NC
        for (unsigned i = 0; i < 5; i += step) {
            std::cout << "[NC] P(" << i << ") = " << evaluate(static_cast<T>(i)) << "\n";
        }
    }
...
int main() {
    ...
    PolynomialEvaluator_NC<int, 3> poly(coeffs);
    poly.demo();
    return 0;
}