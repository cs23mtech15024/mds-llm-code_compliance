...
class SensorCalibrator_C {
public:
    // Compliant mask (uppercase hex letters + uppercase suffix)
    static constexpr uint32_t CALIB_MASK = 0x12BU; // C: uppercase hex suffix and 'U'
    SensorCalibrator_C() : offsets{0, 0, 0, 0} {}

    void applyCalibration(std::vector<uint32_t>& readings) const {
        // Apply mask and compute corrected values
        for (auto &r : readings) {
            // Masking with compliant literal
            r = (r & CALIB_MASK);
        }
    }
    ...
    double computeAverage(const std::vector<uint32_t>& readings) const {
        ...
        uint64_t sum = 0ULL; // C: uppercase 'ULL'
    ...
    void printReport(const std::vector<uint32_t>& readings) const {
        std::cout << "[C] CALIB_MASK = 0x12BU, readings after mask: ";
        for (auto v : readings) std::cout << "0x" << std::hex << std::uppercase << v << " ";
        std::cout << std::dec << "\n";
        std::cout << "[C] Average (raw): " << std::fixed << std::setprecision(2)
