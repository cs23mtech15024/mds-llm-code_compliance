...
// Compliant: mask and flags declared on separate lines (single declarator per declaration)
void access_apply_C(const std::string &logpath = "access_c.log") {
    uint32_t mask = 0u;
    uint32_t flags = 0u;
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        ss << "[C] entry[" << i << "] raw=" << e << " filtered=" << filtered << " permitted=" << std::boolalpha << ok << '\n';
    ...
    fin << std::dec << "[C] allowed=" << allowed << " total=" << entries.size() << '\n';
    std::cout << fin.str();
