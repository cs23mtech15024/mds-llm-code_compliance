...
// Source: MathWorks / Polyspace — MISRA C++:2008 Rule 8-0-1.  [oai_citation:3‡es.mathworks.com](https://es.mathworks.com/help///bugfinder/ref/misrac2008rule801.html?utm_source=chatgpt.com)
// Note: Bit-field layout is implementation-defined; prefer clear, single-member declarations for readability/maintainability.  [oai_citation:4‡SEI Wiki](https://wiki.sei.cmu.edu/confluence/display/c/EXP11-C.%2BDo%2Bnot%2Bmake%2Bassumptions%2Bregarding%2Bthe%2Blayout%2Bof%2Bstructures%2Bwith%2Bbit-fields?utm_source=chatgpt.com)
...
// Non-compliant: two bit-field members declared in one member-declarator-list (seed)
struct Reg {
    unsigned flags:3, mode:5; // NON-COMPLIANT: multiple member-declarators in one member-declarator-list (seed)
    Reg() : flags(0), mode(0) {}
};

// deterministic routine that manipulates registers
void reg_demo_NC(const std::string &logpath = "regbits_nc.log") {
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
        ...
        ss << "[NC] step=" << i << " flags=" << std::dec << r.flags
