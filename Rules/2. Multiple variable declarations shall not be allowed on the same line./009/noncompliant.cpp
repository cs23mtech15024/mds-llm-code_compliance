...
// NON-COMPLIANT: three counters declared on one line (seed).
void process_channels_NC(const std::vector<int>& data, const std::string &logpath = "chan_nc.log") {
    // seed:
    int i = 0, j = 0, k = 0; // NON-COMPLIANT: multiple declarators in one declaration
    ...
    for (size_t w = 0; w + window <= data.size(); w += window) {
        ...
        if (out) out << "[NC] window=" << w/window << " s0=" << sum0 << " s1=" << sum1 << " s2=" << sum2 << '\n';
    ...
    std::cout << std::fixed << std::setprecision(2)
              << "[NC] counts i=" << i << " j=" << j << " k=" << k << " avg=" << avg_events << '\n';
}
