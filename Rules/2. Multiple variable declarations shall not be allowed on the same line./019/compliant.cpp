...
// Compliant: each std::array buffer declared separately (single declarator per declaration)
void dma_ingest_C(const std::string &logpath = "dma_c.log") {
    std::array<int,4> buf1;
    std::array<int,4> buf2;
    ...
    ss << "[C] sum1=" << sum1 << " sum2=" << sum2 << " merged_size=" << merged.size() << '\n';
    std::cout << ss.str();

    std::ofstream out(logpath, std::ios::app);
    if (out) out << ss.str(); else std::cerr << "[C] Failed to open log\n";
