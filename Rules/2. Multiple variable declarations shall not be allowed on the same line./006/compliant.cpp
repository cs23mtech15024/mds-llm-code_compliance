...
// COMPLIANT: each 'auto' declarator is a separate declaration (single init-declarator per declaration).
void counter_simulator_C(const std::vector<int>& samples, const std::string &logpath = "counter_c.log") {
    auto x = 1;
    auto y = 2;
    ...
    for (size_t idx = 0; idx < samples.size(); ++idx) {
        ...
        if (idx % 4 == 0 && out) {
            out << "[C] idx=" << idx << " sample=" << s << " x=" << x << " y=" << y << '\n';
        }
