...
// Compliant: declare each calibration constant on its own line
void calibrate_c(const std::vector<int>& samples, const std::string& logfile = "cal_c.log") {
    int a = 1;
    int b = 2;
    int c = 3;
    ...
    if (!out) std::cerr << "[C] Failed to open calibration log\n";
    ...
    std::cout << "[C] constants: a=" << a << " b=" << b << " c=" << c
