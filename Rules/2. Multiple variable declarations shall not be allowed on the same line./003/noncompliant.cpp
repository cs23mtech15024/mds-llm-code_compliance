...
// Non-compliant: two strings declared on the same line (seed).
void config_load_NC(const std::string &src = "") {
    // seed: std::string s1, s2;
    std::string s1, s2; // NON-COMPLIANT: multiple declarators in one declaration
    ...
    std::ofstream out("config_nc.log", std::ios::app);
    std::ostringstream report;
    report << std::boolalpha << "[NC] cfg s1='" << s1 << "' ok=" << ok1
           << " | s2='" << s2 << "' ok=" << ok2 << '\n';
