...
// Source: MathWorks / Polyspace — MISRA C++:2008 Rule 8-0-1.  [oai_citation:3‡MathWorks](https://it.mathworks.com/help/bugfinder/ref/misrac2008rule801.html?utm_source=chatgpt.com)
...
// Non-compliant: two volatile register variables declared on one line (seed)
void reg_handler_NC(const std::string &logpath = "reg_nc.log") {
    // seed:
    volatile uint32_t regA = 0u, regB = 0u; // NON-COMPLIANT: multiple declarators in one declaration
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
        ...
        ss << "[NC] step=" << i << " op=" << v << " regA=" << static_cast<uint32_t>(regA)
    ...
    fin << std::dec << "[NC] final regA=" << static_cast<uint32_t>(regA)
        << " regB=" << static_cast<uint32_t>(regB) << '\n';
    std::cout << fin.str();
