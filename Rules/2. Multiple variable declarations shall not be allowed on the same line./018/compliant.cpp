...
// Note: SEI CERT DCL04-C also recommends one declaration per variable for clarity.  [oai_citation:6‡SEI External Wiki](https://wiki.sei.cmu.edu/confluence/display/c/DCL04-C.%2BDo%2Bnot%2Bdeclare%2Bmore%2Bthan%2Bone%2Bvariable%2Bper%2Bdeclaration?utm_source=chatgpt.com)
...
// Compliant: each map declared on its own line
void mapping_demo_C(const std::string &logpath = "map_c.log") {
    std::map<int,int> m1;
    std::map<int,int> m2;
    ...
    for (size_t i = 0; i < merged.size(); ++i) {
        if (i % 3 == 0) {
            ...
            ss << "[C] entry[" << i << "] key=" << merged[i].first << " val=" << merged[i].second << '\n';
            std::cout << ss.str();
            if (out) out << ss.str();
        }
    ...
    std::cout << "[C] sizes m1=" << m1.size() << " m2=" << m2.size() << " merged=" << merged.size() << '\n';
    if (out) { out << "[C] sizes m1=" << m1.size() << " m2=" << m2.size() << " merged=" << merged.size() << '\n'; out.close(); }
}
